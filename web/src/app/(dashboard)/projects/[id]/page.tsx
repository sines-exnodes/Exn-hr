...
export default function ProjectDetailPage() {
  ...
  const { data: membersRes, mutate: mutateMembers } = useProjectMembersList(projectId);
  const { data: milestonesRes, mutate: mutateMilestones } = useProjectMilestones(projectId);
  ...
  const handleAddMember = async () => {
    ...
    try {
      ...
      setAddMemberError(err instanceof Error ? err.message : "Thêm thành viên thất bại.");
    } finally {
  ...
  if (loadingProject) {
    return (
      <>
        <Header title="Dự án" breadcrumbs={[{ label: "Dashboard", href: "/" }, { label: "Dự án", href: "/projects" }, { label: "Chi tiết" }]} />
  ...
  if (!project) {
    return (
      <>
        <Header title="Dự án" breadcrumbs={[{ label: "Dashboard", href: "/" }, { label: "Dự án", href: "/projects" }, { label: "Không tìm thấy" }]} />
        <div className="p-6 text-center text-sm text-slate-500">Không tìm thấy dự án.</div>
      </>
    );
  }
  ...
  return (
    <>
                  ...
                  <div className="p-6 space-y-5">
                    ...
                    <div className="flex items-end gap-3 flex-wrap">
                      <div className="flex flex-col gap-1">
                        <label className="text-sm font-medium text-slate-700">Thêm thành viên</label>
                        <select
                      ...
                      <Button variant="primary" loading={addMemberLoading} onClick={handleAddMember}>
                        Thêm
                      </Button>
                    </div>
                    {addMemberError && (
                      ...
                      <div className="overflow-x-auto">
                        <table className="min-w-full">
                          <thead className="bg-slate-50">
                            <tr>
                              {["Nhân viên", "Vai trò", "Ngày tham gia", ""].map((h) => (
                                ...
                                <td className="px-4 py-3">
                                  <div className="flex items-center gap-2">
                                    <div className="flex h-7 w-7 items-center justify-center rounded-full bg-green-100 text-xs font-semibold text-green-600">
                                      {(member.employee?.full_name ?? "?").charAt(0)}
                                    </div>
                                    <span className="text-sm font-medium text-slate-700">
                                      {member.employee?.full_name ?? `NV #${member.employee_id}`}
                                    </span>
                                  </div>
                                </td>
                                <td className="px-4 py-3">
                                  <Badge variant="blue">{roleLabels[member.project_role] ?? member.project_role}</Badge>
                                </td>
              ...
              return (
                <div className="p-6 space-y-4">
                  ...
                  <MilestoneTable
                    ...
                    canManage
                    onEdit={(m) => { setEditingMilestone(m); setShowAddMilestone(true); }}
      ...
      <Modal
        ...
        footer={
          <>
            <Button variant="outline" onClick={() => setDeletingMilestone(null)}>Huỷ</Button>
            <Button variant="danger" loading={deleteLoading} onClick={handleDeleteMilestone}>
              Xoá
            </Button>
          </>
        }
